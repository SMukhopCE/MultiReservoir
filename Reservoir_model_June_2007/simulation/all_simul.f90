...
!                print *, (my_flow_set (iflow_set)%controlled_flows(j))
	...
	call reser_simul(icurrent_id, q, ntime,release,storage_max, storage_ini,rate_area, &
        ...
        write(31,15) icurrent_id, (simul_stor(j), j=1,ntime)        
        15 format(I5,2x, 84(2x,F8.3))
        DO i1 = 1,nchild
...
double precision Unit_Conv
        ...
        Unit_Conv = 62.4*43560*1000/0.74/3600/1000/1000  ! Yi's Unit_Conv = 62.5*1.356*1000/2/31/1000000
        ...
        DO j = 1, ntime
            ...
            ( head_elevation(j) - my_user(iblock_id)%tail_elevation(j,nensemble))*Unit_Conv
...
!		       if(ichild_type.eq.4)release (j) = release(j) + fract*decision_var(ichild_id)
                        if(ichild_type.eq.4)release (j) = release(j) + decision_var((ichild_id*ntime)+j)
