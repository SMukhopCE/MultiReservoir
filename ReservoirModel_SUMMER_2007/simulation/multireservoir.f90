...
external constr_res,expected_benefits,grobfd,grcnfd
...
if(my_user(inum)%user_type.eq.4)then
	...
	my_user(inum)%unit_rate_energy = t6
        ...
        Allocate(my_user(inum)%tail_elevation(ntime, nensem))

        Do k = 1,nensem
                
        		read(14,*)(my_user(inum)%tail_elevation(j,k), j=1,ntime)
        END DO
