...
! Loop for hydropower calculation
        ...
        Do j = 1, 3
                ...
                if  (simul_hydropower(j) .GT. my_user(iblock_id)%installed_capacity)then 
                        simul_hydropower(j) = my_user(iblock_id)%installed_capacity
                end if
        ...
        !print *, 'storage'
        !Do k = 1, 3
     !XXXXXXXX        print *, (simul_stor(k), k=1,3) !XXXXXXXXXXXXXXXXXX
        !END DO 
        
        if ( simul_stor(3) < my_reservoir(icurrent_id)%target_storage) then
                print *, my_reservoir(icurrent_id)%name, ' not meeting target storage'
        end if
...
!        print *, 'power(MW)'
!        Do k = 1, 3
                print *, (simul_hydropower(k), k=1,3)
!        END DO
