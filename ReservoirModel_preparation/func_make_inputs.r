...
###start form here########################################
load("data/TVA.Rdata")
load("data2/TVA_UncontrInQ.Rdata")
load("data2/TVA_StoVol.Rdata")
...
for(f in 1:28){ #28files
  ...
  tmp.file.out <- paste0("output_files/",tmp.reservoir,".dat")
 ...
 # print(tmp.reservoir)
  write (tmp.out.variable, file = paste0("output_files/",tmp.reservoir.out,"_inflow.dat"), ncolumns=length(tmp.NetInflow))
  #print(tmp.reservoir)
...
file.create(paste0("output_files/S_initial.dat"))
file.create(paste0("output_files/S_obs.dat"))
for (f in 1:28){
  ...
  write (tmp.St_1[1], file = paste0("output_files/S_initial.dat"), ncolumns=1, append=TRUE)
  write (tmp.St, file = paste0("output_files/S_obs.dat"), ncolumns=length(tmp.St), append=TRUE)
...
file.create(paste0(,"output_files/TW.dat"))
for (f in 1:28){
  f2 <- reservoir.out.order[f]
  tmp.reservoir = reservoir.names[f2]
  tmp.reservoir.out = reservoir.names.out[f2]
  ...
  write (tmp.TW, file = paste0("output_files/TW.dat"), ncolumns=length(tmp.TW), append=TRUE)
...
##### USER FRACTION #####
file.create(paste0("output_files/usrFract.dat"))
for (f in 1:28){
  f2 <- reservoir.out.order[f]
  tmp.reservoir = reservoir.names[f2]
  tmp.reservoir.out = reservoir.names.out[f2]
  ...
  write(tmp.Fract, file = paste0("output_files/usrFract.dat"), ncolumns=length(tmp.TW), append=TRUE)
}

##### Release Values #####
file.create(paste0("output_files/Releases.dat"))
file.create(paste0("output_files/Releases_obs.dat"))
for (f in 1:28){
  f2 <- reservoir.out.order[f]
  tmp.reservoir = reservoir.names[f2]
  tmp.reservoir.out = reservoir.names.out[f2]
  ...
  tmp.R <-  TotalQ.monthly[[tmp.reservoir]][tmp.ind.R]*date.Num.of.days*24*3600/43560000 # Thousand[acre-ft] in a month
  write (tmp.R, file = paste0("output_files/Releases.dat"), ncolumns=1, append=TRUE)
  
  
  write (tmp.R, file = paste0("output_files/Releases_obs.dat"), ncolumns=length(tmp.R), append=TRUE)
...
file.create(paste0("output_files/ET_coeffs.dat"))
ET.coeffs.all = rep(ET.coeffs, ntime/12)
  write (ET.coeffs.all , file = paste0("output_files/ET_coeffs.dat"), ncolumns= ntime)
